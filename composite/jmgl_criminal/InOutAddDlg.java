...
public class InOutAddDlg extends CriminalInfoDlg{
	 ...
	 * @param style
	 */
	public InOutAddDlg(Shell parent, int style) {
		super(parent, style);
		setText("SWT Dialog");
		createContents();
	}
	...
	public int open() {
		...
		if(oCriminalRecord.isMuti() && afterNum != -1){
			return (oCriminalRecord.getNum() - afterNum);
		}
	...
	protected void createContents() {
		...
		numComposite = new Composite(shell, SWT.NONE);
	...
	public void setData(CriminalRecord criminalRecord){
		oCriminalRecord = criminalRecord;
		if(!criminalRecord.isMuti()) nameText.setText(criminalRecord.getName());
		else numSpinner.setSelection(criminalRecord.getNum());
		...
		numComposite.setVisible(criminalRecord.isMuti());
		
	}

	protected String checkInfo(){
		String errMsg = "";
		if(oCriminalRecord.isMuti()){
			if(numSpinner.getSelection() == 0) return "罪犯人数不得少于1人！";
			else if(numSpinner.getSelection() > oCriminalRecord.getNum()) return "新罪犯人数不得大于原罪犯人数！";
			afterNum = numSpinner.getSelection();
		}
	...
	protected Vector<CriminalRecord> getCriminalRecords(){
		...
		if(!oCriminalRecord.isMuti()){
				...
				criminalRecord.setSex(oCriminalRecord.getSex());
				criminalRecord.setCompany(oCriminalRecord.getCompany());
			...
			criminalRecord.setNum(afterNum);
			criminalRecord.setUuid(oCriminalRecord.getUuid());
			criminalRecord.setSex(oCriminalRecord.getSex());
			criminalRecord.setCompany(oCriminalRecord.getCompany());
			criminalRecord.setState(inButton.getSelection()?DataPropMananger.ST_IN:DataPropMananger.ST_OUT);
